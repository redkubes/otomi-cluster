...
if ye storage; then
  ...
  echo '  storage_account_id=$(az storage account create -n '$storage_account_name' -g '$storage_rg $(y storage.create)' --default-action Deny '$tid $out')'
...
EOF'
  ...
  echo 'az storage account network-rule add -g '$aks_rg' --account-name '$storage_account_name $vnet_subnet_str $out
  ...
  if ye storage.containers; then
    echo 'az storage account update -g '$storage_rg' -n '$storage_account_name' --default-action Allow '$out
    ...
    echo 'az storage account update -g '$storage_rg' -n '$storage_account_name' --default-action Deny '$out
  fi
  echo
fi
